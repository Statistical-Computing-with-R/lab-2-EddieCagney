...
```{r}
#| label: load-data
surveys <- read_csv("C:/Users/Eddie/Documents/STAT 331/Week 2/Lab 2/lab-2-EddieCagney/data/surveys.csv")
```

**2. What are the dimensions (# of rows and columns) of these data?**

There are 30463 observations (rows) and 15 variables (columns) in this dataset

**3. What are the data types of the variables in this dataset?**

There are integers, categorical variables, dates 
...
ggplot(data = surveys,
  ...
  facet_wrap(~species) +
  labs(title = "Animal Diversity Plots", 
       subtitle = "Hind foot Length (mm)", x = "Weight (g)", y = "")
```
...
ggplot(data = surveys,
  ...
  geom_boxplot(outlier.shape = NA)+ geom_jitter(width = 0.1, alpha=0.1, color = "orange3") +
  labs(title = "Animal Diversity Plots", 
       x = "Species",
       y = "Weight") +
...
With an F statistic of 19916 and a p-value less than 2e^-16, we have enough evidence that at least one of the rodent species has a different population mean weight.
